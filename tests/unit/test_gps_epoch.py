...
"""
Regression tests for mixing up the GPS epoch (1980-01-06) with the Unix epoch
(1970-01-01).

CAMM stores GPS time (``CAMMGPSPoint.time_gps_epoch``) while GPX, GPMF and
BlackVue store Unix time (``GPSPoint.epoch_time``). Subtracting one from the
other yields ~315,964,800s, which used to overflow the 32-bit
``segment_duration`` of a version 0 ``elst`` and abort the upload.
"""
...
def _camm_point(time: float, time_gps_epoch: float) -> telemetry.CAMMGPSPoint:
...
class TestEpochConversion:
    ...
    def test_round_trip(self):
        for unix_time in [0.0, 1e9, A_UNIX_TIME, 2e9]:
            assert telemetry.unix_to_gps_epoch(
                telemetry.gps_epoch_to_unix(unix_time)
            ) == pytest.approx(unix_time)
    ...
    def test_leap_seconds_accumulate(self):
        ...
        # 18 by 2026, so the naive +315964800 conversion is 18s too late
        assert (
...
class TestPointAccessors:
    def test_camm_point_exposes_both_epochs(self):
        p = _camm_point(time=0.0, time_gps_epoch=A_GPS_TIME)
        assert p.get_gps_epoch_time() == A_GPS_TIME
        assert p.get_unix_time() == A_UNIX_TIME

    def test_gps_point_exposes_both_epochs(self):
        p = _gps_point(time=0.0, epoch_time=A_UNIX_TIME)
        assert p.get_unix_time() == A_UNIX_TIME
        assert p.get_gps_epoch_time() == A_GPS_TIME

    def test_invalid_timestamps_are_ignored(self):
        assert _camm_point(time=0.0, time_gps_epoch=0.0).get_unix_time() is None
...
class TestGPXOffset:
    ...
    def test_camm_video_syncs_to_zero(self):
        # Same instant, expressed in each container's native epoch
        gpx_points = [_camm_point(time=A_UNIX_TIME, time_gps_epoch=A_GPS_TIME)]
    ...
    def test_real_offset_is_preserved(self):
        gpx_points = [
            _camm_point(time=A_UNIX_TIME + 30, time_gps_epoch=A_GPS_TIME + 30)
        ]
        video_points = [_camm_point(time=0.0, time_gps_epoch=A_GPS_TIME)]
    ...
    def test_missing_video_timestamp_yields_no_offset(self):
        gpx_points = [_camm_point(time=A_UNIX_TIME, time_gps_epoch=A_GPS_TIME)]
        video_points = [_camm_point(time=0.0, time_gps_epoch=0.0)]
        assert GPXVideoExtractor._gpx_offset(gpx_points, video_points) == 0.0
